...
n=22;m=1; %n 是自变量的个数,m 是因变量的个数
x0=X(:,1:n);y0=X(:,n+1); %原始的自变量和因变量数据
e0=data(:,1:n);f0=data(:,n+1); %标准化后的自变量和因变量数据
...
beta3(1,:)=mu(n+1)-mu(1:n)./sig(1:n)*BETA2([2:end],:).*sig(n+1:end); %原始数据回归方程的常数项
beta3([2:n+1])=(1./sig(1:n))'.* repmat(sig(n+1),22,1).*BETA2([2:end],:); %计算原始变量x1,...,xn的系数，每一列是一个回归方程
beta3 = beta3'
fprintf('最后得出如下回归方程：\n')
for i=1%此处为变量y的个数
...
min(abs(er))
mean(abs(er))
ymax=max([yhat;X(:,[n+1])]); %求预测值和观测值的最大值
